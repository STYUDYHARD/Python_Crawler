...
def download_all_urls():
    ...
    for item in soup.find_all(class_="item"):
        movie_url = item.find(class_="pl2").find("a").get("href")
...
def download_movie_view():
    ...
    for url in urls:
        ...
        comments = []
        for page in range(0, 3):  # 设置抓取页数，每页20条
...
json_data1 = json.dumps(datas1_to_write, ensure_ascii=False, indent=4)
with open('movies_data.json', 'w', encoding='utf-8') as f:
    f.write(json_data1)

json_data2 = json.dumps(datas2_to_write, ensure_ascii=False, indent=4)
